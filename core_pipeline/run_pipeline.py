...
def run_diffdock(csv_path, out_dir, diffdock_script, inference_steps, samples, dry_run=False):
    ...
    try:
        result = subprocess.run(cmd, check=True, text=True, cwd=Path(diffdock_script).parent)
        return True
...
def main():
    ...
    success = run_diffdock(args.csv_name, args.out_dir, args.diffdock_path, args.steps, args.samples, args.dry_run)
